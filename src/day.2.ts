...
const partOneStringsFromFile = readFileSync("./src/day.2.txt", {
    encoding: "utf-8",
}).split("\n");

const partTwoStringsFile = readFileSync("./src/day.2.2.txt", {
    encoding: "utf-8",
}).split("\n");
...
const main = async () => {
    ...
    const partOne = (strings = partOneStringsFromFile): number => {
        ...
        type KnownKeys = keyof typeof knowns;

        return strings.reduce((acc, curr) => {
            const [label, ...sets] = curr.split(":");
            const sessionResults = sets
                .flatMap((set) => set.trim().split(";"))
                .map((session) =>
                    session.split(",").map((result) => result.trim())
                )
                .map((session) => {
                    // * Determine how many rgb's are in the session.
                    const rgb = session.reduce(
                        (acc, curr) => {
                        ...
                        {} as Record<KnownKeys, number>
                    );

                    return Object.entries(rgb).every(
                        ([color, count]) => knowns[color] >= count
                    );
                });

            if (sessionResults.length === 0) return acc;

            const valid = sessionResults.every((result) => result === true);

            if (valid === false) return acc;
    ...
    const partTwo = (strings = partTwoStringsFile): number => {
        ...
        return strings.reduce((acc, curr) => {
            const [, ...sets] = curr.split(":");

            let knowns = {};

            sets.flatMap((set) => set.trim().split(";"))
                .map((session) =>
                    session.split(",").map((result) => result.trim())
                )
                .map((session) => {
            ...
            const power = colors.reduce((acc, curr) => {
                const value = knowns[curr];
